...
- **Phase 4** — applied: A9 `837d672` (`tests/test_a11y.py`, `make a11y`, a CI `a11y` job on macos-latest:
  ...
  empty, so there was nothing to shrink). A-left1 `eb472ca` (the Builder's User/Group picker is an ARIA combobox like the board's add field — a listbox of options, `aria-activedescendant`, ArrowDown/Up/Enter/Escape in `builder.js`, a click still picks; a Chrome test walks it by keys and reads the accessibility tree; axe adds it open, still 0). A-left2 `8a71e7e` (a catalog row's Build/Copy and a delegate's Remove name their command or address, visually hidden; the `#busy` pill is `aria-hidden`, no longer a live region saying "Working…" on each request; text fields, selects and textareas take `border-brand-field`, 3:1 on paper and white, held by `test_contrast.py`; a swap that replaced the focused control — the title/department Save, a delegate's Remove — puts focus on its re-rendered twin or the zone, not `<body>`, a Chrome test pressing both by keys; axe still 0). A7 applied after the operator's OK (the two fonts bundled as woff2, `font-src`/`style-src` `'self'`). A8
  declined (D7).
- **Phase 5** — applied: U4 as a text fix (`a9d0a7b`), then as the dashboard (U4a `d652b66`: the tenant, Find a user and three task links; directory and report-bucket counts from the cache with a Load control when it is cold — no `gam` on load but the local `version`; the jobs tray's newest four; the last three failed audit records, linking Audit filtered to failures); U4b-vacation (the user page's Vacation form sends its text as HTML with its line breaks, and pre-fills the text back from the stored body — `lifecycle.autoreply_text`, `6ef6f2e`). Partly: U9
  ...
  page line), axe 0, no JS error — WKWebView and a live tenant unseen) · U7/U1 follow-ups `37b4438` (a user's Groups tab: a chip's × opens the board's confirm step, `/users/groups/remove` runs `guard.enforce(..., confirm_step=True)` and is GATED in the tripwire; its add picker reads the cached `st.groups()` — `tests/test_users_web.py` through the strict mock, axe 0; WKWebView and a live tenant unseen) and the next commit (the Builder's External only counts the tenant's primary, secondary and alias domains and their subdomains as internal: one `gam print domains formatjson` per connector, `AppState.tenant_domains()`, the primary alone if it fails, retried after 10 min; the strict mock answers that shape only — `tests/test_builder.py`; the live tenant's output unseen) · U10a `3989f02` (setup: `static/setup.js`
...
**2026-09-25:** a read-only live pass on the operator's tenant (acceptance, setup's scoped scope check, `print domains`, the admin and scopes read from `oauth2.txt` — all pass; README table updated with the operator's own live writes that day: add group member, remove delegate, set/clear vacation). A 16-CVE machine-scan report triaged: none applies to the shipped interpreters (SECURITY.md). CodeQL's open py/polynomial-redos alert fixed.
