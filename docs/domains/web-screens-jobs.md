...
- Tests only assert a job **started** (a polling panel URL is present); the background task is never driven to completion under `TestClient` (the bg task + mock-gam subprocess can deadlock — it hung CI). Completion is proven separately by deterministic executor tests calling `_run_bulk_store`/`_run_subscribe`/`_run_offboard` directly with fakes. So "the panel rendered" is not "the GAM write works."
