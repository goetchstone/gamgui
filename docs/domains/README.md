...
| [web-screens-jobs](web-screens-jobs.md) | The FastAPI/HTMX screens (Users, Groups, Calendars) and the in-memory job registry (one bounded `Job` base) for polled long-loop progress | `web/jobs.py`, `web/routes/users.py`, `calendars.py`, `groups.py` | #9 (bound anything polled) |
