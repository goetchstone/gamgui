...
def test_run_creates_account_and_returns_sheet(client, tmp_path, monkeypatch):
    ...
    assert 'data-action="copy"' in r.text and 'data-action="print-sheet" data-sheet="ob-creds-sheet"' in r.text
...
def test_credentials_sheets_share_one_print_helper_and_escape_the_copy_text(client):
    # F#23: both one-time sheets use the shared macro + app.js's print-sheet, not their own copy of a print function.
    ...
    assert 'data-sheet="ob-bulk-creds-sheet"' in r.text and "Temp password: COPYpw-1" in r.text
