...
def test_hardening_csp_and_local_assets(client):
    ...
    assert "tailwind-play" not in page.text and "tailwind.config" not in page.text  # no in-browser compiler
    ...
    # the vendored and prebuilt assets are actually served
...
def test_search_messages_runs_and_surfaces_return_path(client):
    ...
    # Asserts on rendered output, not a URL/host check: the fixture's envelope-sender local part is
    # a non-domain token, so this pins the actual Return-Path cell rather than any substring of a host.
    assert "fixture-bounce-0001" in r.text
