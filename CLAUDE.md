...
3. **Only read-only commands may become runnable automatically.** Of 1067 catalog entries, 533 run:
   26 hand-curated (the only ones that can *change* anything) plus 507 grammar-derived commands
...
GAM is pinned at `EXPECTED_GAM_VERSION` in `core/gam/commands.py` (currently 7.46.11); three drift
