...
- **Job control: a Stop button, and retry-only-the-failures.** A bulk loop already stops by itself at
  ...
  failures can only be redone in full. Every job type already tracks its failures; the missing
  pieces are a `cancelled` flag checked in the loop, and a scope that means "just these users."
